...
esp_err_t si7021_send_command(si7021_t device, uint8_t *command, size_t nbytes);
esp_err_t si7021_read_measurement(si7021_t device, uint8_t measure_cmd, float *output, float (*fn)(uint16_t));
...
esp_err_t si7021_setup(si7021_t device, si7021_resolution_t resolution)
{
    ...
    return si7021_write_register(device, resolution << 4);
}
...
esp_err_t si7021_write_register(si7021_t device,uint8_t settings)
{
    ...
    return si7021_send_command(device,data, 2);
}
...
esp_err_t si7021_read_measurement(si7021_t device, uint8_t measure_cmd, float *output, float (*fn)(uint16_t))
{
    ...
    i2c_master_stop(cmd);
    ret = i2c_master_cmd_begin(device.i2c_master_port, cmd, 1000 / portTICK_RATE_MS);
    i2c_cmd_link_delete(cmd);
...
{
    si7021_read_humidity(device, humidity);
    return si7021_read_temperature_after_humidity(device, temperature);
}
...
{
    if (device.temperature_measure_unit == SI7021_FAHRENHEIT)
    {
        return si7021_read_measurement(device, SI7021_COMMAND_READ_TEMP, temperature, &si7021_code_to_fahrenheit);
    }else{
        return si7021_read_measurement(device, SI7021_COMMAND_READ_TEMP, temperature, &si7021_code_to_celsius);
    }
...
{
    if (device.temperature_measure_unit == SI7021_FAHRENHEIT)
        return si7021_read_measurement(device,SI7021_COMMAND_READ_TEMP_AFTER_RH, temperature, &si7021_code_to_fahrenheit);
    else
        return si7021_read_measurement(device,SI7021_COMMAND_READ_TEMP_AFTER_RH, temperature, &si7021_code_to_celsius);
}
...
{
    return si7021_read_measurement(device,SI7021_COMMAND_READ_TEMP, humidity, &si7021_code_to_rh_pct);
}
